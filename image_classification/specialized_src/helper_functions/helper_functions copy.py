...
class OpenImageW:
    ...
    def __getitem__(self, index):
        ...
        for class_name in all_classes:
            if self.mid_to_human_class[class_name] in self.wl_classes:
                output[self.wl_classes.index(self.mid_to_human_class[class_name])] = 1
                print(self.mid_to_human_class[class_name])        
        if output.sum() == 0:
            output[-1] = 1
