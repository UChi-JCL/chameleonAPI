...
class OpenImageW:
    def __init__(self, root, dataset_file, wl_path, transform=None, target_transform=None, start_idx=0, end_idx=0, wl_classes=None):
        self.data_file = pd.read_csv(dataset_file)[start_idx:end_idx]
        self.root = root
        self.ids = []
        ...
        self.end_idx = end_idx
        for i in range(len(self.mid_to_human_class_file)):
            self.mid_to_human_class[self.mid_to_human_class_file.iloc[i][0]] = self.mid_to_human_class_file.iloc[i][1].lower()
        whitelist_mapping = {}
        ...
        for i in range(len(self.data_file)):
            ...
            if self.data_file.iloc[i]['split'] == 'train':
                self.ids.append(self.data_file.iloc[i]['image_id'])
        self.wl_classes = wl_classes
        self.wl_classes_lower = [item.lower() for item in wl_classes]

    def __getitem__(self, index):

        img_id = self.ids[index]
        index = list(self.data_file['image_id']).index(img_id)
        output = torch.zeros(len(self.wl_classes) + 1, dtype=torch.long)
        ...
        for class_name in all_classes:
            if self.mid_to_human_class[class_name] in self.wl_classes_lower:
                output[self.wl_classes_lower.index(self.mid_to_human_class[class_name])] = 1
        if output.sum() == 0:
            output[-1] = 1
