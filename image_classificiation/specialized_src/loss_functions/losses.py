...
#         state = torch.load("/dataheart/yuhanl/Open_ImagesV6_TRresNet_L_448.pth", map_location='cpu')
...
class AsymmetricLossCustomPriorityRankNewLog(nn.Module):
    def __init__(self, stats=None, gamma_neg=4, gamma_pos=1, clip=0.05, eps=1e-8, disable_torch_grad_focal_loss=True,W=None, W_human=None, mapping_dict=None, limit=10, asymm=False, alpha=0.5, alpha1=None, alpha2=None, alpha3=None, penalize_other=False, alpha_other=None, alpha5=1, weight=False, sigmoid = False, ood_weights=None):
        super(AsymmetricLossCustomPriorityRankNewLog, self).__init__()
    ...
    # def our_rank_loss(self, x1, x2, margin):
    #     breakpoint()
    #     return -torch.log(1-(x2-x1+margin))
        # if x2-x1+margin > 0:
    ...
    def forward(self, x, y, y_neg):
        ...
        for i in range(y.shape[0]):
            ...
            if gt_whitelist_index[0] == len(self.all_labels):
                ...
                loss_rank += -torch.log(1-torch.max(all_non_others_vec)) 
                ...
                if len(incorrect_max) > 0:
                    loss_rank += -torch.log(max(correct_max)) 
                    loss_rank += -torch.log(1-max(incorrect_max))
                    # self.our_rank_loss(max(correct_max), max(max(incorrect_max),  top_k_thres), self.alpha1) 
                else:
                    loss_rank += -torch.log(max(correct_max)) 
...
class AsymmetricLossCustomPriorityRankNewNeg(nn.Module):
    def __init__(self, stats=None, gamma_neg=4, gamma_pos=1, clip=0.05, eps=1e-8, disable_torch_grad_focal_loss=True,W=None, W_human=None, mapping_dict=None, limit=10, asymm=False, alpha=0.5, alpha1=None, alpha2=None, alpha3=None, penalize_other=False, alpha_other=None, alpha5=1, weight=False, sigmoid = False, ood_weights=None, wl_list=None):
