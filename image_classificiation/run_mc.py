...
import subprocess
import pandas as pd
all_configs = pd.read_csv("configs/chameleonAPI_MC.csv")
...
spec_configs = json.load(open("configs/specialized_MC.json"))
for i in range(len(all_configs)):
...
for index in ["pretrained", "baseline", "our", "spec"]:
    df = pd.read_csv(f"results/results_multi_choice_{index}.csv")
