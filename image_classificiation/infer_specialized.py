...
import torch
from specialized_src.helper_functions.helper_functions import parse_args
from specialized_src.loss_functions.losses import AsymmetricLossOrigNew, AsymmetricLossCustomPriorityRankNewNeg
from specialized_src.models import create_model
import argparse
import matplotlib
import pandas as pd
matplotlib.use('agg')
...
parser.add_argument('--training_wl', type=str)
...
parser.add_argument('--pretrained', type=str, default="Open_ImagesV6_TRresNet_L_448.pth")
...
def load_all_models(args):
    ...
    for class_file in args.training_wl.split(","):
        class_list_file = pd.read_csv(class_file)["class_name"].tolist()
...
def main():
    ...
    cnt = 0
    all_images = 0
    def find_category(gt_classes, all_class_files):
        all_categories = []
        for gt_class in gt_classes:
            for i, class_file in enumerate(all_class_files):
                if gt_class in class_file:
                    all_categories.append(i)
        return list(set(all_categories))
    for image_name in validation_data_mapping:
        combined_detected_sorted = []
        combined_scores = []
        all_categories = find_category(wl_validation_data_mapping[image_name], all_class_files)
        for i, (model, class_file) in enumerate(zip(all_customized_models, all_class_files)):
            ...
            if i == 0:
                all_images += 1
        sorted_combined = np.array(combined_detected_sorted)[np.argsort(combined_scores)[::-1]][:args.top]
        wrong = calculate_precision_recall_i(args, conf_matrix, image_name, error_dict_file, \
            sorted_combined, gt_wl, gt_wl_neg, result_dict,result_dict_neg, \
                negative_fp, priority_list, donate_num, wl_validation_data_mapping[image_name])
    overall_acc = sum(result_dict['acc']) / all_images
    print(f"{args.wl_path} Overall acc: ", overall_acc)
