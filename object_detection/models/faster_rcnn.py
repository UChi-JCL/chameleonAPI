    ...
    def __init__(
        ...
        **kwargs,
        ...
        roi_heads = RoIHeads(
            ...
            box_score_thresh,
            box_nms_thresh,
             box_detections_per_img, mapping_dict=mapping_dict, our_loss=our_loss
        )
...
def _fasterrcnn_mobilenet_v3_large_fpn(
    ...
    **kwargs: Any,
    ...
    import os
    if weights is not None:
        if os.environ["WL_EVAL"] == "True" or os.environ['NEW'] == "True":
            state = weights.get_state_dict(progress=progress)
            pretrained_dict = {k: v for k, v in state.items() if k in model.state_dict() and "roi_heads" not in k}
