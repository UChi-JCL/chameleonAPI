...
def get_coco_label_names():
    ...
    coco_index_to_name_mapping = {}
    coco_name_to_index_mapping = {}
    for i in coco_class_ids:
        coco_index_to_name_mapping[i] = coco_label_names[i]
        coco_name_to_index_mapping[coco_label_names[i]] = i

    coco_cls_colors = np.random.randint(128, 255, size=(80, 3))

    return coco_label_names, coco_class_ids, coco_name_to_index_mapping, coco_index_to_name_mapping



def our_rank_loss(x1, x2, margin):
    ALPHA2 = int(os.getenv('ALPHA2'))
    ALPHA3 = int(os.environ.get('ALPHA3'))
    alpha2 = int(ALPHA2) 
    alpha3 = int(ALPHA3)
    if x2-x1+margin > 0:
...
def map_orig_coco_labels_to_wl_labels(labels, mapping_dict):
    ...
    all_wl_names =list( mapping_dict.values())
    for label in labels:
        label_name = coco_index_to_name_mapping[label]
        if label_name not in all_wl_names:
            new_labels += [len(all_wl_names)]
        else:
            label_index = all_wl_names.index(label_name)
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, mapping_dict=None, our_loss=False):
    ...
    # labels = map_orig_coco_labels_to_wl_labels(labels, mapping_dict)
    ...
    if our_loss == 'True':
        ...
        index_arr = np.array(mapping_dict['wl1'])
        class_logits_n[:,0] = torch.max(class_logits[:, index_arr], dim=1)[0]
    
        others_arr = np.setdiff1d(others_indices, np.array(mapping_dict['wl1']))   
        class_logits_n[:, 1] = torch.max(class_logits[:, others_arr], dim=1)[0]
        ...
        for i in range(len(labels)):
            ...
            TH = float(os.environ.get('TH'))
            if labels[i] in mapping_dict['wl1']:
                classification_loss[i] = our_rank_loss(class_logits_n[i][0], max(TH, class_logits_n[i][1]), 0)
                # targets_n[i][priority_list.index(key)] = 1
            else:
                classification_loss[i] = our_rank_loss( class_logits_n[i][1], max(TH, class_logits_n[i][0]), 0)
        # classification_loss = F.cross_entropy(class_logits_n, labels_n.long())
        classification_loss = classification_loss.mean()
    elif os.environ['BASE'] == "1":
        classification_loss = torch.zeros(len(labels)).cuda()
        for i in range(len(labels)):
            # if labels[i] in mapping_dict['wl1']:
            #     labels_n[i] = 0
            ...
            TH = float(os.environ.get('TH'))
            if labels[i] in mapping_dict['wl1']:
                classification_loss[i] = F.cross_entropy(class_logits[i], labels[i]) * 3
                # targets_n[i][priority_list.index(key)] = 1
            else:
                classification_loss[i] = F.cross_entropy(class_logits[i], labels[i]) 
        # classification_loss = F.cross_entropy(class_logits_n, labels_n.long())
        classification_loss = classification_loss.mean()
        
    else:
        
        classification_loss = F.cross_entropy(class_logits, labels) #, weight=torch.tensor([5, 1]).float().cuda()) 
...
class RoIHeads(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            loss_classifier, loss_box_reg = fastrcnn_loss(class_logits, box_regression, labels, regression_targets, mapping_dict=self.mapping_dict, our_loss =self.our_loss)
