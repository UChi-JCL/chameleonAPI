...
ALPHA2 = os.getenv('ALPHA2')
ALPHA3 = os.environ.get('ALPHA3')
def our_rank_loss(x1, x2, margin):
    alpha2 = int(ALPHA2) 
    alpha3 = int(ALPHA3)
    if x2-x1+margin > 0:
    ...
    # loss_func = nn.MarginRankingLoss(margin=0.1)
    # target = torch.tensor([1]).cuda()
    # return loss_func(x1.unsqueeze(0), x2.unsqueeze(0), target)
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, mapping_dict=None, our_loss=False):
    # type: (Tensor, Tensor, List[Tensor], List[Tensor]) -> Tuple[Tensor, Tensor]
    """
    Computes the loss for Faster R-CNN.
    ...
    if our_loss == 'True':
        ...
        targets_n = torch.zeros((class_logits.shape[0], len(mapping_dict) - 1)).cuda()
        class_logits_n =  torch.zeros((class_logits.shape[0], len(mapping_dict) )).cuda()
        labels_n =  torch.zeros(class_logits.shape[0]).cuda()
        m = nn.Sigmoid()
        class_logits = m(class_logits)
        others_indices = np.arange(class_logits.shape[1])
        for key in mapping_dict:
            if key != 'other':
                index_arr = torch.tensor(np.array(mapping_dict[key])).cuda()
                class_logits_n[:, priority_list.index(key)] = torch.max(class_logits[:, index_arr], dim=1)[0]
            else:
                index_arr = np.setdiff1d(others_indices, np.array(mapping_dict['wl1']))   
                class_logits_n[:, priority_list.index(key)] = torch.max(class_logits[:, index_arr], dim=1)[0]

        classification_loss = torch.zeros(len(labels)).cuda()
        th = 0.5
        for i in range(len(labels)):
            if labels[i] in mapping_dict['wl1']:
                labels_n[i] = 0
            else:
                labels_n[i] = 1
                    
            
            # if labels[i] in mapping_dict[key]:
            #     classification_loss[i] = our_rank_loss(class_logits_n[i][0], class_logits_n[i][1], 0.1)
            #     # targets_n[i][priority_list.index(key)] = 1
            # else:
            #     classification_loss[i] = our_rank_loss( class_logits_n[i][1], class_logits_n[i][0], 0.1)
        classification_loss = F.cross_entropy(class_logits_n, labels_n.long())
...
class RoIHeads(nn.Module):
    ...
    def __init__(
        ...
        #     ('fc1', nn.Linear(bs * 512 * 91, 200 * bs))
        # ]))
    def has_mask(self):
    ...
    def postprocess_detections(
        ...
        all_scores = []
        all_labels = []
        all_scores_orig_shape = []
        for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
            boxes = box_ops.clip_boxes_to_image(boxes, image_shape)
            scores_orig_shape = scores.clone()
            ...
            boxes, scores, labels = boxes[keep], scores[keep], labels[keep]
            # all_scores_orig_shape.append(scores_orig_shape[keep, :])
            all_boxes.append(boxes)
            all_scores.append(scores)
            all_labels.append(labels)

        return all_boxes, all_scores, all_labels, all_scores_orig_shape
    def pad(self, class_logits):
        bs = 1

        class_logits_n = torch.zeros((bs * 512, 91)).cuda()
        if len(class_logits) < bs * 512:
            
            class_logits_n[:len(class_logits), :] = class_logits
        else:
            class_logits_n[:bs * 512, :] = class_logits[:bs * 512, :]
        return class_logits_n
    def forward(
        self,
        ...
        if self.training:
            ...
            #     fastrcnn_loss(class_logits, box_regression, labels, regression_targets, self.mapping_dict, self.our_loss)
            loss_classifier, loss_box_reg = \
                fastrcnn_loss(class_logits, box_regression, labels, regression_targets, mapping_dict= self.mapping_dict)
            
            losses = {"loss_classifier": loss_classifier, "loss_box_reg": loss_box_reg}
            # class_logits= self.pad(class_logits)
            # wl1_logits = self.fc(class_logits.view(-1))

            # wl1_target = [len(torch.where(labels_n == self.mapping_dict['wl1'][0])[0]) \
            #     + len(torch.where(labels_n == self.mapping_dict['wl1'][1])[0]) \
            #         + len(torch.where(labels_n == self.mapping_dict['wl1'][2])[0]) for labels_n in labels]
            # our_loss = 0
            
            # # transformed_wl1_logits = [wl1_logits[i * 100: (i+1) * 100] for i in len(labels)]
            # for i in range(len(labels)):
            #     ce_loss = nn.CrossEntropyLoss()
            #     our_loss += ce_loss(wl1_logits[i * 200: (i+1) * 200].unsqueeze(0), torch.tensor(min(199, wl1_target[i]), dtype=torch.long).cuda().unsqueeze(0))

            
            # losses["loss_classifier"] = our_loss / len(labels)
            
        else:
            boxes, scores, labels, scores_orig_shape = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
            num_images = len(boxes)
            class_logits_n = self.pad(class_logits)
            # scores_count = self.fc(class_logits_n.view(-1).unsqueeze(0))
