...
class CocoDetection(torchvision.datasets.CocoDetection):
    def __init__(self, img_folder, ann_file, transforms, extra_input=None, split='train'):
        super(CocoDetection, self).__init__(img_folder, ann_file)
        self._transforms = transforms
        if extra_input:
            filtered_id = pd.read_csv(extra_input)
            filtered_id = list(filtered_id['image_id'][filtered_id['split'] == split])
            self.ids = filtered_id
...
def get_coco(args, split, root, image_set, transforms, mode='instances', extra_input=None):
    ...
    dataset = CocoDetection(img_folder, ann_file, transforms=transforms, extra_input=extra_input, split=split)
    # if image_set == "train":
