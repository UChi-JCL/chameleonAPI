...
class DataTrainingArguments:
    ...
    app_name: Optional[str] = field(
        default=None,
        metadata={
            "help": "The name of the app to use for specialized classification."
        },
    )
...
def main():
    ...
    num_labels = len(name_to_tc_mapping[os.environ['SAVE']][0]) + 1
    ...
    #     new_preds = [0 for _ in range(len(tc_mapping) + 1)]
    #     breakpoint()
    #     for i in range(len(preds)):
    #         for j in tc_mapping[i]:
    #             if j in labels:
    #                 transformed_label += [i]
    ...
    def categorize(tc_mapping, preds):
        ...
        tc_mapping = name_to_tc_mapping[os.environ['SAVE']]
        new_preds = []
        for i in range(len(preds)):
            new_label = [0 for _ in range(len(tc_mapping[0]) + 1)]
            for c in range(len(tc_mapping[0])):
                if preds[i][tc_mapping[0][c]] == 1:
                    new_label[c] = 1
            if sum(new_label) == 0:
                new_label[-1] = 1
            new_preds.append(new_label)
        return new_preds
    ...
    if training_args.do_train:
        ...
        # trainer.save_state()
        torch.save({"model_state_dict": model.state_dict()}, f"checkpoints/{os.environ['SAVE']}.pt")
