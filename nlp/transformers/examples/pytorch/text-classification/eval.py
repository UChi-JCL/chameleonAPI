...
def get_label_list(raw_dataset, split="train") -> List[str]:
    ...
    if isinstance(raw_dataset[split]["label_name"][0], list):
        label_list = [label for sample in raw_dataset[split]["label_name"] for label in sample]
        label_list = list(set(label_list))
    else:
        label_list = raw_dataset[split].unique("label_name")
...
def main():
    ...
    if training_args.do_train or training_args.do_eval and not is_regression:  # classification, training
    ...
    def multi_labels_to_ids(labels: List[str]) -> List[float]:
        ...
        return ids
    def categorize(tc_mapping, preds):
        new_preds = []
        for item in preds:
    ...
    def preprocess_function(examples):
        ...
        if label_to_id is not None and "label" in examples:
            if is_multi_label:
                result["label"] = [multi_labels_to_ids(l) for l in examples["label_name"]]
            else:
                result["label"] = [(label_to_id[str(l)] if l != -1 else -1) for l in examples["label"]]
        if os.environ['OUR'] == "True":
            tc_mapping = [[-1, 0], [0, 1]]
            result['label'] = categorize(tc_mapping, result['label'])
        return result
    ...
    def compute_metrics_ours(p):
        preds = p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
        preds = np.argmax(preds, axis=1)
        tc_mapping = [[-1, 0], [0, 1]]
        if os.environ['OUR'] == "False":
            preds = categorize(tc_mapping, preds)
        # new_labels = categorize(tc_mapping, p.label_ids)
        result = metric.compute(predictions=preds, references=p.label_ids)
        return result
    ...
    trainer = Trainer(
        ...
        compute_metrics=compute_metrics_ours,
        tokenizer=tokenizer,
    ...
    metrics = trainer.evaluate(eval_dataset=eval_dataset, resume_from_checkpoint=False)
    # metrics = trainer.evaluate(eval_dataset=eval_dataset, resume_from_checkpoint=True, output_dir=training_args.output_dir)
